    ...
    private List<ItemPropertyType> getCategoryRelatedItemProperties(List<Category> categories, int rowIndex) throws TemplateParseException {
        ...
        for (Category category : categories) {
            for (Property property : category.getProperties()) {
                ...
                Integer columnIndex = findCellIndexForProperty(property.getPreferredName());
                if(columnIndex == null) {
                    continue;
                }
                ...
                List<Object> values = (List<Object>) parseCell(cell,property.getPreferredName(), property.getDataType(), true);
                if (values.isEmpty()) {
                    continue;
                }
